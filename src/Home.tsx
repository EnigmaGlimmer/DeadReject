...
import {getDataByMint, insertData, getDataByStatus, getOrInsertData, getDataByOwner, updateData} from './DbLink';
...
const Home = (props: HomeProps) => {
    ...
    const onMint = async (
        ...
        try {
            if (wallet.connected && candyMachine?.program && wallet.publicKey) {
                ...
                const ownerstatus = await updateData(wallet.publicKey.toBase58(), wallet.publicKey?.toBase58(), 3);
                setIsMinting(true);
                ...
                if(nftData.other.length > 0) {
                    const data = await getDataByStatus(2);
                    const mintaddr = data.data[0]?.mint;
                    const ownerstatus = await getOrInsertData(wallet.publicKey.toBase58());
                    // console.log("data", nftData.other);
                    console.log("owner status", ownerstatus)
                    let index = 0, hindex = 0;
                    if(ownerstatus == 3 && nftData.other.length > 0) {
                        transactions.push(new Transaction());
                        await Promise.all(nftData.other?.map(async (item: any, keyIndex: any) => {
                            console.log("h transactions", transactions, hindex, keyIndex, index);
                        ...
                        await updateData(wallet.publicKey.toBase58(), wallet.publicKey.toBase58(), 4);
                    }
                }
    ...
    const isMinted = async (data: any) => {
        const result = await getDataByMint(data.mint);
        if(result.ok == false) {
            return {ok: false, flag: false}
        }
        if(!result.data) {
            await insertData(data.mint, wallet.publicKey, 0)
